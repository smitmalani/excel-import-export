...
import { MemberSystemType } from '../members/enums/member-system-type.enum';
import { Gender } from '../members/enums/gender.enum';
import {
  BadRequestException,
  Injectable,
...
export class ExcelImportExportService {
  ...
  async exportExcel(res: Response, count: number, businessId) {
    try {
      ...
      members.forEach((m, index) => {
        const row = worksheet.addRow({
          ...
          SmartCardNumber:
            m && m.SmartCardNumber ? `'${m.SmartCardNumber}` : '',
      ...
      res.setHeader(
        'Content-Disposition',
        'attachment; filename=member_list.xlsx',
      );
      ...
      throw new InternalServerErrorException('Could not generate Excel file');
    }
  }
  ...
  async inportExcel(worksheet, currentUser) {
    ...
    try {
      ...
      const cardTypeMap = new Map(
        allCardTypes.map((card) => [card.CardName, card.CardTypeID]),
      );
      ...
      let smartCardCounter = parseInt(
        latestMember?.SmartCardNumber ?? '0000000000000000',
      );
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        if (!firstName || !lastName || !mobile || !email) return;
      ...
      if (unknownCardTypes.length > 0) {
        ...
        throw new BadRequestException(`Card type not found: ${unique}`);
      }
      ...
      for (const member of members) {
        ...
        const data = {
          ...
          MemberType: MemberType
            ? { set: MemberType as MemberSystemType }
            : undefined,
          Gender: Gender ? { set: Gender as Gender } : null,
        };
        ...
        if (MemberID) {
          ...
          const newMember = await this.prisma.member.create({
            data: {
              ...
              MemberType:
                (member.MemberType as MemberSystemType) ||
                MemberSystemType.Patient,
            },
            include: { Business: true, HeldCardType: true },
          });

          let cardTypeName = newMember.HeldCardType?.CardName ?? 'N/A';
          const memberLoyaltyCard = await generateLoyaltyCard(
            newMember,
            cardTypeName,
          );
        }
      ...
      if (err instanceof PrismaClientKnownRequestError) {
        console.log(err);
        throw new BadRequestException('Please download lates file');
      } else {
        console.log('Error while importing file', err);
        throw new InternalServerErrorException('Error while importing file');
      }
    }
  }
}
